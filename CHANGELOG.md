...
- *Your subscription* admin page: enter the licence key that comes with a Moodle Marketplace order, and the site registers with the whiteboard service through LTI 1.3 Dynamic Registration.
- The key is kept, so the page can register again on its own if the service drops an invite that was never used.
